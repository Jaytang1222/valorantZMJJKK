...
export default function VersusPage() {
  ...
  useEffect(() => {
    if (room?.phase === "playing")
      router.replace(`/versus/match?code=${encodeURIComponent(room.code)}`);
  ...
  return (
    <main className="game-shell">
      ...
      {!room && (
        <section className="versus-entry">
          {recoverCode && (
            <div className="recover-room-actions">
              <button
                className="secondary-action"
                onClick={() =>
                  router.push(
                    `/versus/match?code=${encodeURIComponent(recoverCode)}`,
                  )
                }
              >
                恢复对局
      ...
      {room?.phase === "lobby" && (
        <section className="game-board">
          ...
          <div className="room-actions">
            <button
              disabled={!connected || currentMember?.status !== "connected"}
              onClick={() =>
                run("room:ready", {
                  code: room.code,
