...
export function createRealtimeServer(httpServer: HttpServer): Server {
  ...
  const ownGuesses = (room: LiveRoom, userId: string) => room.members.find((member) => member.userId === userId)?.guesses ?? [];
  const emitRoom = (room: LiveRoom) => io.to(`room:${room.code}`).emit("room:state", publicRoom(room));
  const emitClosed = (room: LiveRoom) => io.to(`room:${room.code}`).emit("room:closed", { code: room.code });

  const answerNameFor = async (room: LiveRoom) => {
    if (room.answerName || !room.targetPlayerId) return room.answerName;
    const [target] = await db.select({ canonicalName: players.canonicalName }).from(players).where(eq(players.id, room.targetPlayerId)).limit(1);
  ...
  const emitFinishedRoom = async (room: LiveRoom) => {
    ...
    await saveRoom(room);
    emitRoom(room);
    if (answerName) io.to(`room:${room.code}`).emit("room:round-result", { room: publicRoom(room), target: { canonicalName: answerName } });
  ...
  const scheduleRoundTimeout = (code: string, endsAt: number) => {
    setTimeout(async () => {
      ...
      try {
        release = await acquireRoomLock(code);
        const room = await loadRoom(code);
        if (!room || room.phase !== "playing" || room.roundEndsAt !== endsAt) return;
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("match:join", async (input, acknowledge) => {
      ...
      try {
        ...
        io.to(opponent.socketId).emit("match:found", { room: publicRoom(room) });
        acknowledge?.({ room: publicRoom(room), ownGuesses: ownGuesses(room, userId) });
        emitRoom(room);
      } catch (error) {
    ...
    socket.on("room:join", async ({ code }, acknowledge) => {
      ...
      try {
        ...
        acknowledge?.({ error: error instanceof Error ? error.message : "Unable to join room" });
      } finally {
        await release?.();
      }
    });
    ...
    socket.on("room:ready", async ({ code, ready }, acknowledge) => {
      ...
      try {
        ...
        acknowledge?.({ error: error instanceof Error ? error.message : "Unable to update ready state" });
      } finally {
        await release?.();
      }
    });
    ...
    socket.on("room:start", async ({ code }, acknowledge) => {
      ...
      try {
        ...
        acknowledge?.({ error: error instanceof Error ? error.message : "Unable to start room" });
      } finally {
        await release?.();
      }
    });
    ...
    socket.on("room:guess", async ({ code, playerId }, acknowledge) => {
      ...
      try {
        ...
        acknowledge?.({ error: error instanceof Error ? error.message : "Unable to submit guess" });
      } finally {
        await release?.();
      }
    });
    ...
    socket.on("room:surrender", async ({ code }, acknowledge) => {
      ...
      try {
        ...
        acknowledge?.({ error: error instanceof Error ? error.message : "Unable to surrender" });
      } finally {
        await release?.();
      }
    });
    ...
    socket.on("room:leave", async ({ code }, acknowledge) => {
      ...
      try {
        ...
        acknowledge?.({ error: error instanceof Error ? error.message : "Unable to leave room" });
      } finally {
        await release?.();
      }
    });
    ...
    socket.on("room:rematch", async ({ code }, acknowledge) => {
      ...
      try {
        ...
        acknowledge?.({ error: error instanceof Error ? error.message : "Unable to request rematch" });
      } finally {
        await release?.();
      }
    });
    ...
    socket.on("room:reconnect", async ({ code }, acknowledge) => {
      ...
      try {
        ...
        acknowledge?.({ error: error instanceof Error ? error.message : "Unable to reconnect" });
      } finally {
        await release?.();
      }
    });
    ...
    socket.on("disconnect", async () => {
      ...
      for (const key of keys) {
        ...
        try {
          release = await acquireRoomLock(code);
          const room = await loadRoom(code);
          if (!room || !room.members.some((member) => member.userId === userId && member.status === "connected")) continue;
          const roomSockets = await io.in(`room:${room.code}`).fetchSockets();
          if (roomSockets.some((candidate) => candidate.data.userId === userId)) continue;
          disconnectMember(room, userId);
          await saveRoom(room);
          emitRoom(room);
          io.to(`room:${room.code}`).emit("room:member-disconnected", { userId, reconnectDeadline: Date.now() + 20_000 });
