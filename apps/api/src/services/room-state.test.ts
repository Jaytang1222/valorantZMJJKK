...
const wrongGuess = (id: string) => ({
  id,
  playerId: `player-${id}`,
  canonicalName: "Wrong",
  comparison: { country: "mismatch" },
  isCorrect: false,
  points: 0,
});
...
describe("room state", () => {
  it("requires two ready players and limits each member to eight guesses", () => {
    ...
    for (let index = 0; index < 8; index += 1)
      recordGuess(room, "host", wrongGuess(`guess-${index}`), 1);
    expect(() => recordGuess(room, "host", wrongGuess("guess-9"), 1)).toThrow(
      "Guess limit reached",
    );
