...
if (confirmation !== "RUN_STAGING_LOAD")
  throw new Error("Set LOAD_CONFIRM=RUN_STAGING_LOAD to run the load test");
if (
  !apiUrl.startsWith("https://") &&
  process.env.ALLOW_INSECURE_LOAD !== "true"
)
  throw new Error(
...
async function post(path: string, body: unknown, token?: string) {
  ...
  if (!response.ok)
    throw new Error(`${path}: ${String(data.error ?? response.status)}`);
  return data;
}

async function createTicket(index: number) {
  const email = `m2-load-${runId}-${index}@example.test`;
  const registered = (await post("/v1/auth/register", { email, password })) as {
...
function emit(socket: Socket, event: string, payload: object) {
  return new Promise<RoomReply>((resolve, reject) => {
    socket
      .timeout(12_000)
      .emit(event, payload, (error: Error | null, reply: RoomReply) => {
...
async function runRoom(index: number) {
  ...
  try {
    ...
    const [hostTicket, guestTicket] = await timed("auth_and_ticket", async () =>
      Promise.all([createTicket(index * 2), createTicket(index * 2 + 1)]),
    );
    [host, guest] = await timed("socket_connect", async () =>
      Promise.all([connect(hostTicket), connect(guestTicket)]),
    );
    const created = await timed("room_create", () =>
      emit(host!, "room:create", {
        maxPlayers: 2,
        roundCount: 1,
    ...
    await timed("room_surrender", () =>
      emit(guest!, "room:surrender", { code }),
    );
    ...
    failures.push(
      `room ${index}: ${error instanceof Error ? error.message : String(error)}`,
    );
  } finally {
    host?.disconnect();
    guest?.disconnect();
...
let nextRoom = 0;
await Promise.all(
  Array.from({ length: Math.min(workers, roomCount) }, async () => {
    while (true) {
